...
PyLandGaus::PyLandGaus()
{
    chisqr = 0.0;
    ndf=0;
    pllo[0]=0.1; pllo[1]=0.; pllo[2]=1.0; pllo[3]=0.4;  // edit here to change boundaries of fit parameters
...
void PyLandGaus::langaus(char* fname, double* range, bool offset, double* range0) {
  ...
  //  TH1F* hSNR = new TH1F("hSNR","Signal-to-noise",xbins_,xmin_,xmax_);
    TH1F* hSNR = new TH1F("hSNR",fname,xbins_,xmin_,xmax_); // edit plot name

    double ped0,ped1,ped2;
    if (offset) {
        ...
        hPed->Fit("myfit","RL");
        ped0= myfit.GetParameter(0);
        ped1= myfit.GetParameter(1);
        ped2= myfit.GetParameter(2);
        voffset = myfit.GetParameter(1);  // Here is the place to edit to put in pedestal by hand.
      //  voffset = -1.082;  // Hardwire in the pedestal -- remember to comment out when done!
    ...
    fr[0]=0.3*hSNR->GetMean();   //  override this to set low end of fit range
    // fr[0]=0.2;   // hardwired in low end of fit range
   ...
   TLatex *myt = new TLatex(0,0,Form("pedestal = %0.2f",voffset));
   myt ->SetTextFont(42);
